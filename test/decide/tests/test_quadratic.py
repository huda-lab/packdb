"""Tests for convex quadratic programming (QP) objectives.

Covers:
  - POWER(linear_expr, 2) syntax
  - ** operator form
  - (expr)*(expr) multiplication form
  - MINIMIZE enforcement (MAXIMIZE rejected)
...
class TestQuadraticErrors:
    ...
    def test_maximize_with_power_rejected(self, packdb_cli):
        """MAXIMIZE SUM(POWER(x, 2)) is non-convex and must be rejected."""
