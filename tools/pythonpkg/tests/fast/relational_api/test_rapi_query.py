...
def scoped_default(duckdb_cursor):
    default = packdb.connect(':default:')
    packdb.set_default_connection(duckdb_cursor)
    # Overwrite the default connection
    yield
    # Set it back on finalizing of the function
    packdb.set_default_connection(default)
...
class TestRAPIQuery(object):
    ...
    def test_query_non_select_result(self, duckdb_cursor):
        with pytest.raises(packdb.ParserException, match="syntax error"):
    ...
    def test_set_default_connection(self, scoped_default):
        packdb.sql("create table t as select 42")
        assert packdb.table('t').fetchall() == [(42,)]
        ...
        packdb.set_default_connection(con2)

        with pytest.raises(packdb.CatalogException, match='Table with name d does not exist'):
            con2.table('d').fetchall()

        assert packdb.table('t').fetchall() == [(21,)]

        packdb.sql("create table d as select [1,2,3]")

        assert packdb.table('d').fetchall() == [([1, 2, 3],)]
    ...
    def test_set_default_connection_error(self, scoped_default):
        with pytest.raises(TypeError, match='Invoked with: None'):
            ...
            packdb.set_default_connection(None)

        with pytest.raises(TypeError, match='Invoked with: 5'):
            packdb.set_default_connection(5)

        assert packdb.sql("select 42").fetchall() == [(42,)]
        packdb.close()

        with pytest.raises(packdb.ConnectionException, match='Connection Error: Connection already closed!'):
            packdb.sql("select 42").fetchall()
        ...
        con3 = packdb.connect()
        con3.close()
        packdb.set_default_connection(con3)
