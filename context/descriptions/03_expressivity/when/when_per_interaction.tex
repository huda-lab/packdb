...
\lstdefinelanguage{DECIQL}{
    keywords={SELECT,FROM,WHERE,DECIDE,IS,BOOLEAN,INTEGER,REAL,SUCH,THAT,
              AND,OR,NOT,WHEN,PER,SUM,MIN,MAX,AVG,
...
SUM over an empty set evaluates to $0$. Whether this causes a problem
...
\begin{enumerate}[leftmargin=2em]
    \item \textbf{SUM} with a lower bound or equality ($\geq$, $=$)
    \item \textbf{MIN/MAX} with an existential meaning (``some row must satisfy \ldots'')
\end{enumerate}

Upper-bound SUM and universal MIN/MAX (``every row must satisfy
\ldots'') are immune.
...
This works for SUM with lower bounds (Section~5.1), existential MAX,
...
\midrule
SUM, AVG &
    Sum of no rows is 0. AVG scales
...
\begin{itemize}[leftmargin=2em]
    \item \textbf{Inner SUM}: phantom 0 absorbed. Works.
