...
Two separate test suites verify that `EXPLAIN`, `EXPLAIN ANALYZE`, and `EXPLAIN (FORMAT JSON)` produce correct output for the DECIDE operator:

- **C++ sqllogictest** (`test/sql/decide_explain.test`): 30+ regex-based checks on inline data. Covers the DECIDE node name, Variables/Objective/Constraints sections, WHEN/PER display, MINIMIZE variant, multiple variables, JSON format keys, EXPLAIN ANALYZE timing, and both logical and physical plan output.
- **Python pytest on TPC-H** (`test/decide/tests/test_explain.py`): 21 tests using real TPC-H tables (`lineitem`, `supplier`, `partsupp`). Covers WHEN (string filters, numeric comparisons, mixed constraints), PER (basic, integer variables), WHEN+PER combined, JSON format, EXPLAIN ANALYZE, and logical plan output. Run via `./test/decide/run_tests.sh -m explain`.
