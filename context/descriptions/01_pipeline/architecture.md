...
graph TD
    ...
    subgraph PackDB Execution
        ...
        Model --> Solver[Solver: Gurobi / HiGHS]
...
3.  **Solve Phase**: The constructed model is passed to `ILPModel::Build()` which creates a solver-agnostic representation, then `SolveILP()` dispatches to the best available backend (Gurobi preferred, HiGHS fallback).
